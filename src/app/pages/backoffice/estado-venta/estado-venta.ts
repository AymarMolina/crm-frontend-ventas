...
@Component({
  ...
  imports: [VentaForm,CommonModule,FormsModule],
  templateUrl: './estado-venta.html',
  styleUrl: './estado-venta.css',
})
export class EstadoVenta  implements OnInit {
  ...
  cargarTodos(): void {
    this.loading = true;
    this.ventasService.listar({ page: 0, size: 10000 }).subscribe({
      next: (res) => {
  ...
  private _aplicarFiltrosYPaginar(pagina: number): void {
    ...
    if (texto) {
      resultado = resultado.filter(v =>
        ...
        v.agenteNombre?.toLowerCase().includes(texto) 
      );
    }
